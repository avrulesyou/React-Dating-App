...
const SexuallyInterestedInScreen = (props) => {
              ...
              <View style={styles.View7c604ee7}>
                <Touchable
                  onPress={() => {
                    const handler = async () => {
                      try {
                        const newSexualInterest = setGlobalVariableValue({
                          key: "sexual_interest",
                          value: "male",
                        });
                        const updatedUser =
                          await xanoAPIEditUserInfoPOST.mutateAsync({
                            ...
                            name: fetchData?.name,
                            sexual_interest: newSexualInterest,
                            user_city: fetchData?.user_city,
                          });
                        setUser({ ...user, ...updatedUser });
                        navigation.navigate("CareerInfoScreen");
                      } catch (err) {
                        console.error(err);
                      }
                    };
...
export default withTheme(SexuallyInterestedInScreen);
