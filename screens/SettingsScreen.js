...
const SettingsScreen = (props) => {
  ...
  const xanoAPIEditUserSettingsPOST = XanoAPIApi.useEditUserSettingsPOST();
  ...
  React.useEffect(() => {
    setMaximum_age(
      user?.maximum_age ?? GlobalVariables.AGE_PREFENRENCE.MAXIMUM_AGE
    );
    setMinimum_age(
      user?.minimum_age ?? GlobalVariables.AGE_PREFENRENCE.MINIMUM_AGE
    );

    // Update State and Ref of Distance Slider
    setSliderValue(user?.distance_preference ?? 0);
    distanceSliderRef.current.setNativeProps({
      value: user?.distance_preference ?? 0,
    });
  }, []);

  const handleOnSave = async () => {
    try {
      const updatedUser = await xanoAPIEditUserSettingsPOST.mutateAsync({
        distance_preference: sliderValue,
